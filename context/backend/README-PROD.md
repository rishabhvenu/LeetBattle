...
- `K8S_JUDGE0_WORKER_MEMORY_REQUEST` - Default: 1Gi (recommended: 1Gi for Java compilation)
- `K8S_JUDGE0_WORKER_MEMORY_LIMIT` - Default: 4Gi (recommended: 4Gi for Java compilation)
- `K8S_JUDGE0_WORKER_CPU_REQUEST` - Default: 1000m (recommended: 500m)
- `K8S_JUDGE0_WORKER_CPU_LIMIT` - Default: 2000m (recommended: 2000m for Java compilation)
...
- `BOT_LEADER_TTL_MS` - Redis lease window for leader election (default 15000)
...
- **Session Affinity**: Not required—Redis-backed presence keeps matchmaking and rooms in sync across replicas.

### MongoDB Sharded Cluster
- **Type**: LoadBalancer (via Mongos routers)
- **Port**: 27017 (standard port)
- **Architecture**: 
  - **Config Servers**: 3 replicas (internal, port 27019)
  - **Mongos Routers**: 2+ replicas (LoadBalancer, port 27017)
  - **Shard Servers**: 3 shards, each with 3 replicas (internal, port 27018)
- **Internal URI**: `mongodb://username:password@mongodb-mongos:27017/codeclashers?authSource=admin`
- **External URI**: `mongodb://username:password@<mongos-loadbalancer-ip>:27017/codeclashers?authSource=admin`
- **Connection**: Always connect via Mongos routers (never directly to shards)
- **Authentication**: Required (configure via `MONGODB_USERNAME` and `MONGODB_PASSWORD` secrets)
- **Scaling**: Add more shards horizontally to scale storage and performance
- **High Availability**: Each shard is a replica set with automatic failover

### Redis Cluster
- **Type**: LoadBalancer
- **Port**: 6379 (standard port)
- **Architecture**: 
  - **Nodes**: 6+ nodes (3 masters + 3 replicas minimum)
  - **Mode**: Redis Cluster with automatic sharding and failover
- **Internal**: Cluster nodes accessible via `redis-cluster-headless` service
- **External**: LoadBalancer provides single entry point to cluster
- **Connection**: Use cluster-aware Redis client (ioredis with cluster mode)
- **Configuration**:
  - `REDIS_CLUSTER_ENABLED=true` (required)
  - `REDIS_HOST=<loadbalancer-ip>` or `REDIS_CLUSTER_NODES=node1:6379,node2:6379,...`
  - `REDIS_PORT=6379`
  - `REDIS_PASSWORD=<password>`
  - `REDIS_USERNAME=<username>` (optional)
  - `REDIS_TLS=true` for TLS-enabled clusters (use `REDIS_TLS_REJECT_UNAUTHORIZED=false` for self-signed certs)
- **Scaling**: Add more nodes to cluster for increased capacity
