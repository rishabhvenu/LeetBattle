...
kubectl port-forward -n "$NAMESPACE" svc/mongodb-dev 27017:27017 > /dev/null 2>&1 &
echo "✅ MongoDB:    localhost:27017"

kubectl port-forward -n "$NAMESPACE" svc/redis 6379:6379 > /dev/null 2>&1 &
echo "✅ Redis:      localhost:6379"

kubectl port-forward -n "$NAMESPACE" svc/colyseus 2567:2567 > /dev/null 2>&1 &
echo "✅ Colyseus:   localhost:2567"

kubectl port-forward -n "$NAMESPACE" svc/judge0-server 2358:2358 > /dev/null 2>&1 &
echo "✅ Judge0:     localhost:2358"

kubectl port-forward -n "$NAMESPACE" svc/minio-dev 9000:9000 > /dev/null 2>&1 &
echo "✅ MinIO API:  localhost:9000"

kubectl port-forward -n "$NAMESPACE" svc/minio-dev 9001:9001 > /dev/null 2>&1 &
echo "✅ MinIO UI:   localhost:9001"

kubectl port-forward -n "$NAMESPACE" svc/grafana 3030:3030 > /dev/null 2>&1 &
