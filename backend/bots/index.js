...
const { createHealthServer } = require('./lib/healthServer');
const { circuitBreakers } = require('./lib/apiClient');
...
async function main() {
  ...
  try {
    ...
    const healthPort = parseInt(process.env.BOT_HEALTH_PORT || '3000', 10);
    healthServer = createHealthServer({
      port: healthPort,
      leadership,
      getStats: () => getBotDeploymentStats(redis),
      circuitBreakers
    });
